...
public class CTMBManualEntry extends ManualEntry
{
	...
	private HashMap<String, IData> dataSources = new HashMap<>();
	...
	public void loadTexts(boolean updateMeta)
	{
		...
		File manualEntries = new File(new File(new File(CommonProxy.resourceLoader.getResourceFolder(), CTMB.MODID), "ie_manual"), l.getLanguageCode().toLowerCase());
		File stateFile = new File(manualEntries, getName()+".md"); //i.e. ie_manual/pl_pl/melter.md

		if(nonEnglish&&!stateFile.exists())
		{
			manualEntries = new File(new File(new File(CommonProxy.resourceLoader.getResourceFolder(), CTMB.MODID), "ie_manual"), "en_us");
